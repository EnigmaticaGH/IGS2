...
public class GrabBlock : MonoBehaviour
{
    ...
    void Start()
    {
        controller = GetComponentInParent<ControllerNumber>();
        block = null;
        axisInUse = false;
        carryingBlock = false;
    }
    ...
    {
        foundBlock = false;
        Ray ray = new Ray(transform.position + Vector3.right * transform.localScale.x + Vector3.down * 1.5f, Vector3.up);
        Ray ray2 = new Ray(transform.position + Vector3.left * transform.localScale.x * 1.2f + Vector3.down * 1.5f, Vector3.up);
    ...
    void Update()
    {
        ...
        if (carryingBlock)
        {
            ...
            block.transform.position = transform.position + Vector3.right * transform.localScale.x * 0.75f;
        }
    }
    ...
    void ThrowBlock()
    {
        ...
        Vector3 force = Vector3.right * blockThrowForce * transform.localScale.x + Vector3.up * (BASE_VERTICAL_FORCE + verticalForce);
